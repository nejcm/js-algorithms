...
It starts traversing from a selected node (source or starting node) and traverse
...
DFS is a edge based technique. It uses the Stack data structure, performs two
stages, first visited nodes are pushed into stack and second if there is no node
then visited nodes are popped.
