...
describe('DisjointSet', () => {
  it('should create disjoint set', () => {
    ...
    expect(disjointSet.items).toBeDefined();
    expect(disjointSet.items.size).toEqual(0);
  });
  ...
  it('should add and find items in set', () => {
    ...
    expect(disjointSet.items.get('c')?.value).toEqual('test');
  });
  ...
  it('should make union from set', () => {
    ...
    expect(disjointSet.items.get(1)?.rank).toEqual(1);

    disjointSet.union(1, 2);

    expect(disjointSet.items.get(1)?.rank).toEqual(2);
    ...
    expect(disjointSet.items.get(1)?.rank).toEqual(3);
    expect(disjointSet.find(1)).toEqual(1);
    expect(disjointSet.find(2)).toEqual(1);
  ...
  it('should do collapsing find', () => {
    ...
    expect(disjointSet.items.get(2)?.rank).toEqual(4);
    expect(disjointSet.items.get(5)?.parent?.key).toEqual(4);
    expect(disjointSet.find(5)).toEqual(2);
    expect(disjointSet.items.get(5)?.parent?.key).toEqual(2);
  });
});
